...
def value_list_for_ramp(ramp: RampConf, reverse=False):
    'Get the list of values for a RampConf object'
    if reverse:
...
def input_ramp(parameter_name, amplifier_name) -> RampConf:
    'Get the parameters of a voltage ramp from the keyboard'
...
''')
    ...
    with open(__file__, 'rt') as fin:
        this_script = fin.readlines()

    print(comment_lines)

    # Save the recipe and include this script in the comments
    with open(recipe_file_name, "wt") as fout:
        recipe.write_to_file(fout, comment_lines=comment_lines,
